...
public class Print {
  ...
  void prExp(BINOP e, int d) {
    ...
    case CJUMP.EQ: say("EQ CJUMP"); break;
    case CJUMP.LT: say("LT CJUMP"); break;
	default:
  ...
  void prExp(CALL e, int d) {
     indent(d); sayln("CALL(");
	prExp(e.func,d+1);
        for(ExpressionList a = e.args; a!=null; a=a.tail) {
           sayln(","); prExp(a.head,d+2);
        }
        say(")");
  }

  void prExp(Expression e, int d) {
  ...
  public void prStm(Stm s) {prStm(s,1); say("\n");}
  public void prExp(Expression e) {prExp(e,1); say("\n");}
